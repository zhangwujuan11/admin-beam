  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="桥梁名称" prop="bridgeName">
        <el-input v-model="queryParams.bridgeName" placeholder="请输入桥梁名称" clearable size="small"
          @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item label="左右幅" prop="leftRightSpan">
        <el-input v-model="queryParams.leftRightSpan" placeholder="请输入左右幅" clearable size="small"
          @keyup.enter.native="handleQuery" />
      </el-form-item>
      ...
      <el-form-item label="梁片尺寸" prop="beamDimension">
        <el-input v-model="queryParams.beamDimension" placeholder="请输入梁片尺寸" clearable size="small"
          @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item label="施工工法" prop="constructionMethod">
        <el-input v-model="queryParams.constructionMethod" placeholder="请输入施工工法" clearable size="small"
          @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item label="施工工序" prop="constructionProcess">
        <el-input v-model="queryParams.constructionProcess" placeholder="请输入施工工序" clearable size="small"
          @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item label="施工班组" prop="constructionTeam">
        <el-input v-model="queryParams.constructionTeam" placeholder="请输入施工班组" clearable size="small"
          @keyup.enter.native="handleQuery" />
      </el-form-item>
      ...
      <el-form-item label="台座编号" prop="seatNumber">
        <el-input v-model="queryParams.seatNumber" placeholder="请输入台座编号" clearable size="small"
          @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item label="模板编码" prop="templateCode">
        <el-input v-model="queryParams.templateCode" placeholder="请输入模板编码" clearable size="small"
          @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item label="检验日期" prop="inspectionDate">
        <el-date-picker clearable size="small" v-model="queryParams.inspectionDate" type="date" value-format="yyyy-MM-dd"
          placeholder="选择检验日期">
        </el-date-picker>
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
          v-hasPermi="['beam:inspection:add']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate"
          v-hasPermi="['beam:inspection:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete"
          v-hasPermi="['beam:inspection:remove']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" :loading="exportLoading" @click="handleExport"
          v-hasPermi="['beam:inspection:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="inspectionList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="主键ID" align="center" prop="processInspectionId" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body :close-on-click-modal="false">
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="检验日期" prop="inspectionDate">
          <el-date-picker clearable size="small" v-model="form.inspectionDate" type="date" value-format="yyyy-MM-dd"
            placeholder="选择检验日期">
          </el-date-picker>
        </el-form-item>
...
<script>
import { listInspection, getInspection, delInspection, addInspection, updateInspection, exportInspection } from "@/api/beam/inspection";
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange (selection) {
      this.ids = selection.map(item => item.processInspectionId)
    ...
    handleUpdate (row) {
      this.reset();
      const processInspectionId = row.processInspectionId || this.ids
      getInspection(processInspectionId).then(response => {
    ...
    submitForm () {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.processInspectionId != null) {
    ...
    /** 删除按钮操作 */
    handleDelete (row) {
      const processInspectionIds = row.processInspectionId || this.ids;
      this.$modal.confirm('是否确认删除工序检验编号为"' + processInspectionIds + '"的数据项？').then(function () {
        return delInspection(processInspectionIds);
      }).then(() => {
        this.getList();
    ...
    handleExport () {
      ...
      this.$modal.confirm('是否确认导出所有工序检验数据项？').then(() => {
